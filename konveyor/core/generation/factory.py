...
class ResponseGeneratorFactory:
    ...
    @classmethod
    def get_generator(cls, generator_type: str = "default", config: Optional[Dict[str, Any]] = None) -> ResponseGeneratorInterface:
        """
        ...
        # Get configuration
        openai_client_type = config.get("openai_client_type", "unified") if config else "unified"
        ...
        if generator_type == "default":
            ...
            if not context_service:
                logger.warning("Context service not provided for RAG generator, using default generator")
            ...
            if not conversation_service:
                logger.warning("Conversation service not provided for chat generator, using default generator")
                return cls.get_generator("default", config)
    ...
    @classmethod
    def register_generator(cls, generator_type: str, generator: ResponseGeneratorInterface) -> None:
        """
