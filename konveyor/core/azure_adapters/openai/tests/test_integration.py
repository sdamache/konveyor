...
def test_completion_generation():
    ...
    for i, messages in enumerate(test_messages):
        try:
            ...
            logger.info(f"Successfully generated completion with {len(completion)} chars")
            print(f"\nPrompt {i+1}:")
            print(f"User: {messages[-1]['content']}")
            print(f"AI: {completion[:100]}..." if len(completion) > 100 else f"AI: {completion}")
        except Exception as e:
            logger.error(f"Failed to generate completion for message set {i+1}: {str(e)}")
...
def verify_environment():
    ...
    for var in required_vars:
        ...
        if not value:
            ...
            logger.info(f"Environment variable {var} is set to: {value if 'KEY' not in var else '****'}")
