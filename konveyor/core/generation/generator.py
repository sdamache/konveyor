...
class ResponseGenerator(ResponseGeneratorInterface):
    ...
    def __init__(
        ...
        self.openai_client = OpenAIClientFactory.get_client(
            client_type=openai_client_type,
    ...
    async def generate_with_rag(self, query: str,
        ...
        if not self.context_service:
            logger.warning("Context service not available, falling back to direct generation")
            return await self.generate_direct(
                query=query,
                conversation_id=conversation_id,
                **kwargs
    ...
    def format_prompt(self, template_type: str,
                     context: str,
                     query: str,
                     **kwargs) -> Dict[str, str]:
        """
    ...
    async def _update_conversation_history(self, conversation_id: str, query: str, response: str) -> None:
        """
