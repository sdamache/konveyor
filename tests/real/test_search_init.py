...
def check_environment_variables():
    ...
    if missing_vars:
        ...
        logger.error(
            "Please ensure these are exported in your shell session before running."
        )
        ...
        if os.path.exists(env_path):
            ...
            )  # Don't override shell vars
            missing_vars = [
                var for var in required_vars if not os.getenv(var)
            ]  # Recheck
            if missing_vars:
                logger.error(
                    f"Still missing after .env check: {', '.join(missing_vars)}"
                )
                sys.exit(1)
            else:
    ...
    logger.info(
        f"Using AZURE_OPENAI_API_VERSION: {os.getenv('AZURE_OPENAI_API_VERSION')}"
    )
