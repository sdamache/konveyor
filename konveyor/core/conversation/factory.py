...
class ConversationManagerFactory:
    ...
    @staticmethod
    async def create_manager(storage_type: str = "memory", config: Optional[Dict[str, Any]] = None) -> ConversationInterface:
        """
        ...
        if storage_type == "memory":
            ...
            if not cosmos_conn_str or not redis_conn_str:
                logger.warning("Missing Azure storage configuration, falling back to in-memory storage")
                return InMemoryConversationManager()
            ...
            try:
                ...
                # Redis is not available, fall back to in-memory storage
                logger.warning("Redis package not available, falling back to in-memory storage")
                return InMemoryConversationManager()
            except Exception as e:
                # Other errors, fall back to in-memory storage
                logger.warning(f"Error creating Azure storage manager: {str(e)}, falling back to in-memory storage")
                return InMemoryConversationManager()
