...
async def test_conversation_viewset():
    ...
    with patch('konveyor.apps.rag.views_updated.AzureClientManager') as MockClientManager, \
         patch('konveyor.apps.rag.views_updated.RAGService') as MockRAGService, \
         patch('konveyor.core.conversation.factory.ConversationManagerFactory.create_manager') as mock_create_manager, \
         patch('konveyor.core.formatters.factory.FormatterFactory.get_formatter') as mock_get_formatter:
        ...
        mock_rag_service.generate_response = AsyncMock(return_value={
            ...
            'sources': [{'source': 'test_source.txt', 'page': 1, 'relevance_score': 0.95}],
            'context_chunks': [{'content': 'Test context', 'source': 'test_source.txt'}],
        ...
        mock_conversation_manager.create_conversation.return_value = {"id": "test_conversation_id", "user_id": "test_user"}
        mock_conversation_manager.get_user_conversations.return_value = [{"id": "test_conversation_id", "user_id": "test_user"}]
        mock_conversation_manager.get_conversation_messages.return_value = [
            {"id": "test_message_id", "conversation_id": "test_conversation_id", "content": "Test query", "type": "user"},
            {"id": "test_message_id2", "conversation_id": "test_conversation_id", "content": "Test response", "type": "assistant"}
        ]
        mock_create_manager.return_value = mock_conversation_manager
        
        # Create mock formatter
        mock_formatter = MagicMock()
        mock_formatter.format_message.return_value = {"text": "Formatted message", "blocks": []}
        ...
        mock_conversation_manager.create_conversation.assert_called_once_with('test_user')
        ...
        mock_conversation_manager.get_user_conversations.assert_called_once_with('test_user')
        
        # Test ask
        request = factory.post('/api/rag/conversations/test_conversation_id/ask/', {'query': 'What is the capital of France?'})
        request.user = MagicMock(is_authenticated=True, id='test_user')
