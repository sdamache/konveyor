...
async def test_in_memory_conversation_manager():
    ...
    message = await manager.add_message(
        conversation_id=conversation["id"], content="Hello, world!", message_type="user"
    )
    ...
    string_context = await manager.get_conversation_context(
        conversation["id"], format="string"
    )
    dict_context = await manager.get_conversation_context(
        conversation["id"], format="dict"
    )
    openai_context = await manager.get_conversation_context(
        conversation["id"], format="openai"
    )
    ...
    updated_conversation = await manager.update_conversation_metadata(
        conversation_id=conversation["id"], metadata={"test_key": "test_value"}
    )
