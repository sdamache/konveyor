...
async def test_generate_response_kubernetes_concepts(rag_service):
    ...
    rag_service.openai_client.chat.completions.create.return_value = AsyncMock(
        choices=[AsyncMock(message=AsyncMock(content="A Pod in Kubernetes is the smallest deployable unit..."))]
    )
...
async def test_generate_response_linux_kernel(rag_service):
    ...
    rag_service.openai_client.chat.completions.create.return_value = AsyncMock(
        choices=[AsyncMock(message=AsyncMock(content="System calls in Linux provide..."))]
    )
...
async def test_generate_response_with_code_examples(rag_service):
    ...
    rag_service.openai_client.chat.completions.create.return_value = AsyncMock(
        choices=[AsyncMock(message=AsyncMock(content="Here's an example of a Pod YAML:\n```yaml\napiVersion: v1..."))]
    )
...
async def test_generate_response_with_multiple_sources(rag_service):
    ...
    rag_service.openai_client.chat.completions.create.return_value = AsyncMock(
        choices=[AsyncMock(message=AsyncMock(content="Kubernetes containers use Linux namespaces for isolation..."))]
    )
