...
class AzureOpenAIClient:
    ...
    def __init__(
        ...
        self.endpoint = endpoint or os.getenv("AZURE_OPENAI_ENDPOINT", "https://eastus.api.cognitive.microsoft.com/")
        self.embeddings_api_version = embeddings_api_version or os.getenv("AZURE_OPENAI_EMBEDDING_API_VERSION", "2023-05-15")
        self.gpt_api_version = gpt_api_version or os.getenv("AZURE_OPENAI_API_VERSION", "2024-12-01-preview")
        self.gpt_deployment = gpt_deployment or os.getenv("AZURE_OPENAI_GPT_DEPLOYMENT", "gpt-deployment")
        self.embeddings_deployment = embeddings_deployment or os.getenv("AZURE_OPENAI_EMBEDDING_DEPLOYMENT", "embeddings")
    ...
    def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.info(f"Generating embedding for text of length {len(truncated_text)}")
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 404:
                    ...
                    logger.error(f"Current deployment name: {self.embeddings_deployment}")
    ...
    def generate_completion(self, messages: List[Dict[str, str]], max_tokens: int = 1000) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Successfully generated completion of length {len(content)}")
                return content
            else:
                error_msg = f"Completion Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                raise Exception(error_msg)
