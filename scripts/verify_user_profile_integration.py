...
def verify_user_profiles():
    ...
    if profile_count == 0:
        logger.error(
            "No user profiles found. Integration may not be working correctly."
        )
        return False
    ...
    stats = {
        ...
        "with_code_language": SlackUserProfile.objects.exclude(
            code_language_preference=""
        ).count(),
        "with_response_format": SlackUserProfile.objects.exclude(
            response_format_preference=""
        ).count(),
        "avg_interactions": SlackUserProfile.objects.aggregate(
            Avg("interaction_count")
        )["interaction_count__avg"],
        "max_interactions": SlackUserProfile.objects.aggregate(
            Max("interaction_count")
        )["interaction_count__max"],
    }
    ...
    for profile in SlackUserProfile.objects.all():
        ...
        logger.info(
            f"    Code language: {profile.code_language_preference or 'Not set'}"
        )
        logger.info(
            f"    Response format: {profile.response_format_preference or 'Not set'}"
        )
