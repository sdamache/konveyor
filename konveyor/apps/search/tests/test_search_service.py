...
class SearchServiceTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        if missing_vars:
            print(f"Skipping tests due to missing environment variables: {', '.join(missing_vars)}")
        ...
        # Prepare test files
        cls.test_files_dir = Path(__file__).resolve().parent.parent.parent / 'documents' / 'tests' / 'test_files'
    ...
    def test_01_create_search_index(self):
        ...
        required_fields = {
            ...
            'embedding': {'type': SearchFieldDataType.Collection(SearchFieldDataType.Single)}
        }
        ...
        for field_name, properties in required_fields.items():
            ...
            for prop_name, prop_value in properties.items():
                if prop_name != 'vectorSearchProfile':
                    self.assertEqual(getattr(field, prop_name), prop_value, 
    ...
    def test_02_document_indexing(self):
        ...
        with open(test_file, 'rb') as f:
            ...
            embedding = self.indexing_service.search_service.generate_embedding(content[:1000])
