...
class ChunkService(AzureService):
    ...
    def __init__(self, max_chunk_size: int = 5000, max_batch_size: int = 100):
        ...
        self.log_success(f"Initialized with max_chunk_size={max_chunk_size}, max_batch_size={max_batch_size}")
    ...
    def calculate_batch_size(self, chunks: List[DocumentChunk]) -> int:
        ...
        if avg_size > self.max_chunk_size:
            ...
            optimal_size = min(
                self.max_batch_size,
                int(self.max_batch_size * (size_ratio / 10))
            )
            
        self.log_debug(f"Calculated optimal batch size: {optimal_size} for avg chunk size: {avg_size:.0f}")
        return optimal_size
    ...
    def process_in_batches(
        ...
        self.log_debug(f"Processing {total_chunks} chunks in batches of {actual_batch_size}")
        
        # Yield batches
        for i in range(0, total_chunks, actual_batch_size):
            batch = chunks[i:i + actual_batch_size]
            self.log_debug(f"Processing batch {i//actual_batch_size + 1} with {len(batch)} chunks")
            yield batch
    ...
    def validate_chunk(self, chunk: DocumentChunk) -> bool:
        ...
        try:
            ...
            if len(chunk.content) > self.max_chunk_size:
                self.log_warning(f"Chunk {chunk.id} exceeds max size: {len(chunk.content)} > {self.max_chunk_size}")
                return False
                
            # Validate metadata
            if not isinstance(chunk.metadata, dict):
                self.log_warning(f"Chunk {chunk.id} has invalid metadata type: {type(chunk.metadata)}")
                return False
